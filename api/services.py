...
class VoxERPService:
    ...
    def _pending(self, identity: Identity, auth: dict[str, Any], filters: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"reply_text": f"Mark student {pending['student_id']} {pending['status']} in {pending['subject']}, period {period} on {pending['date']}. Say yes or no.", "pending": pending, "requires_confirmation": True, "status": 200}
