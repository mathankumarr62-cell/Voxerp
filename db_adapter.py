...
def get_timetable(student_id: str, year: Optional[int] = None, semester: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "ok", "rows": [{"day": r["day"], "section": r["section"], "year": r["year"], "semester": r["semester"], "period": r["period"], "course_id": r["course_id"]} for r in rows], "message": f"Timetable for {student_id}: {', '.join(f'{r["day"]} Period {r["period"].split('_')[1]}' for r in rows[:5])}{'...' if len(rows) > 5 else ''}"}
    finally:
        _close_connection(conn)
